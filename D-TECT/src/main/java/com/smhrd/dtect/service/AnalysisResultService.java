...
public class AnalysisResultService {
	...
	public String getUsernameForAnalId(Long analId) {
		return analysisRepository.findById(analId).map(Analysis::getUser).map(User::getUserIdx)
				.flatMap(userRepository::findMemberUsernameByUserId).orElse("");
	}

	/** analId → name (표시명/실명) */
	public String getNameForAnalId(Long analId) {
		return analysisRepository.findById(analId).map(Analysis::getUser).map(u -> u.getMember()).map(m -> m.getName())
				.orElse("사용자");
	}
	...
	public long appendTypes(String sid, List<FieldName> types, Long total) {
		if (sid == null || sid.isBlank())
			throw new IllegalArgumentException("sid 누락");
	...
	public AnalysisStatusDto getStatus(String sid) {
		SessionState st = sessions.get(sid);
		if (st == null)
			return new AnalysisStatusDto(0, 0, null);
	...
	public AnalRate gradeByCounts(Map<FieldName, Integer> counts) {
		...
		Map<FieldName, Integer> W = Map.of(FieldName.VIOLENCE, 1, FieldName.DEFAMATION, 1, FieldName.SEXUAL, 1,
				FieldName.BULLYING, 1, FieldName.CHANTAGE, 1, FieldName.EXTORTION, 1);
		int weighted = 0;
		...
		if (weighted >= 30)
			return AnalRate.DANGER;
		if (weighted >= 15)
			return AnalRate.WARNING;
		return AnalRate.NORMAL;
	}
	...
	public void saveFromCallback(Long analId, List<Map<String, Object>> results) {
		if (analId == null)
			throw new IllegalArgumentException("analId is null");
	...
	public boolean finalizeByAnalId(Long analId) {
		...
		return pdfWebhookClient.dispatchCountsWithAnalId(analId, username, name, sid, counts, rate, started, ended);
	}

	/** 세션 파이널 — 웹훅에 username+name 모두 전달 */
	public boolean finalizeNow(String sid) {
		...
		boolean ok = pdfWebhookClient.dispatchCountsWithAnalId(st.analId, username, name, sid, counts, rate,
	...
	private Map<FieldName, Integer> parseCountsFromAnalResult(String json) {
		try {
			...
			if (countsNode == null || countsNode.isNull())
				countsNode = node.get("typeCounts");
			if (countsNode != null && countsNode.isObject()) {
				...
				for (var e : strMap.entrySet()) {
					...
					if (fn != null)
						out.put(fn, Math.max(0, e.getValue() == null ? 0 : e.getValue()));
				}
				return out;
			}
		...
		for (ModelMessage m : results) {
			...
			for (LabelCount lc : m.getClassification()) {
				if (lc == null || lc.getCount() <= 0)
					continue;
				try {
					FieldName fn = FieldName.valueOf((lc.getLabel() == null ? "" : lc.getLabel()).toUpperCase());
					Case c = new Case();
