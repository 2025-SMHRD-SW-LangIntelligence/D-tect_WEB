...
public class AnalysisPdfUploadController {
    ...
    @Transactional
    public ResponseEntity<PdfCallbackResponse> upload(
            @RequestParam(required = false) String sid,
            @RequestParam(required = false) Long userId, // Long로 받되, 없으면 sid로 복원
        ...
        if (userId == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new PdfCallbackResponse(null, "unauthorized: userId missing"));
        }
        ...
        String key = writer.uploadAutoName(
                (sid == null || sid.isBlank()) ? String.valueOf(userId) : sid,
                originalName,
                file.getBytes(),
                contentType
        );
